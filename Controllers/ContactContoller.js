...
export const ADD_POST = async (req, res) => {
  try {
    ...
    await res
      .status(200)
      .send({ msg: "add with success", contact: newContact });
  } catch (error) {
...
export const Get_ele_byID = async (req, res) => {
  try {
    ...
    if (oneContact) {
      res.status(200).send({ msg: "find ID with succes", contact: oneContact });
    } else {
      res
        .status(200)
        .send({ msg: "there is no contact with these information" });
    }
  } catch (error) {
